...
const checkResourceCache = async (filename, fn) => {
	if (!(filename in resource_cache)) {
		resource_cache[filename] = fn(filename);
	}
...
export const initialize = async () => {
	font_types = Object.assign({}, ...Object.entries(await font_types).map(([a, b]) => ({
		[b]: a
	})));
...
export function* getDependencies(dialog_string) {
	...
	for (let box of modified_string.split('\n\n')) {
		let second_line = false;
		for (let line of box.split('\n')) {
			...
			for (let l of split_line) {
				...
				strip_end = second_line ? -2 : -1;
				second_line = !second_line;
			}
			// Strip the unnecessary 'last' newline set if applicable
			final_string = final_string.slice(0, strip_end);
		}
		final_string += '\n\n'
	}
	// Remove the unnecessary final double newline
	...
	for (let idx in text_boxes) {
		...
		for (let key in necessary_fonts[idx]) {
			necessary_fonts[idx][key] = checkResourceCache("resources/fonts/" + necessary_fonts[idx][key] + ".png", load1bppFontAsync);
		}
...
export const drawBox = async (dependencies) => {
	...
	for (let key in necessary_fonts) {
		necessary_fonts[key] = await necessary_fonts[key];
	}
	...
	for (let line_number in text) {
		...
		for (let character_idx = 0; character_idx < line.length; character_idx++) {
			...
			if (character == '<') {
				...
				let special_data = "";
				while (line[++character_idx] != '>') {
					special_data += line[character_idx];
				}
				switch (control_character) {
					case 'f':
						current_font = necessary_fonts[parseInt(special_data)];
						break;
					default:
						assert(false, "Unknown control code: " + control_character);
				}
				continue;
			}
